#!/bin/bash
##############################################################################################
# Starts ads-qa tests on ads docker with custom genesis file.
##############################################################################################
...
if [ -f "$FILE_GENESIS" ]; then
    ...
    cp $FILE_GENESIS ${PATH_DOCKERIZATION_REPO}/development/ads/docker/genesis.json
else
